...
var sammyApp = Sammy('#app_main', function() {
  ...
  this.get('#/', function(context) {
    if(auth()) {
      // cordova.plugins.snackbar('Welcome back ' + getUserData('name') + '!', 'LONG', '', function(){});
  ...
  this.get('#/orders/:table', function(context) {
    if (auth()) {
      ...
      this.load('templates/header.hb').then(function(partial) {
        ...
        context
        .load(apiUrl + '/orders/waiter/' + context.table_id, {
          headers: {
            'Authorization': 'Bearer ' + getLocal('api_token')
          }
        })
        .then(function(response) {
          context.tableOrder = JSON.parse(response);
          // context.tableOrder = {
          //   "ordered_by": 3,
...
function quickOrder() {
  navigator.notification.confirm('This action cannot be undone', confirm, 'Place this order?', ['Cancel', 'Dine in']);
  //, 'Take Out'
}
